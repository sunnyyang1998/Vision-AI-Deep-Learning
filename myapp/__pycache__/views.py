...
from .models import Order, Product, OrderReminder, Customer, Spicy, Paralysis
...
def menu(request, table_id):
    ...
    # 添加辣度和麻度级别到上下文中
    spice_levels = Spicy.objects.values_list('id', flat=True)
    numbness_levels = Paralysis.objects.values_list('id', flat=True)
    ...
    context = {
        ...
        'spice_levels': list(spice_levels),  # 添加到上下文
        'numbness_levels': list(numbness_levels),  # 添加到上下文
    }
...
def your_order_page(request, table_id):
    ...
    for product_id, quantity in shopping_cart.items():
        try:
            product = Product.objects.get(ProductsID=product_id)
...
def orders(request):
    ...
    # Group unfinished orders by table ID and calculate total price for each table
    tables = Order.objects.filter(is_checkout=False).values('table_id').annotate(total_price=Sum('total_price'))
    table_totals = []
    for table in tables:
        table_id = table['table_id']
        total_price = Order.objects.filter(table_id=table_id, is_checkout=False).aggregate(total_price=Sum('total_price'))['total_price']
        total_price = round(total_price, 2)  # 四舍五入到两位小数
        table_totals.append({'table_id': table_id, 'total_price': total_price})
    
    spicy = Spicy.objects.all()
    paralysis = Paralysis.objects.all()
    spicy_data = zip(spicy, range(1, len(spicy) + 1))
    paralysis_data = zip(paralysis, range(1, len(paralysis) + 1))
    
    context = {
        'orders': unfinished_orders,
        'tables': tables,
        'spicy': spicy_data,
        'paralysis': paralysis_data,
...
def delete_order(request, order_id):
    ...
    order.delete()
    return redirect('orders')  # assuming you have a view named 'order_list' that shows all orders
...
def product_add(request):
    if request.method == "POST":
        ...
        if form.is_valid():
            form.save()
            return redirect('product_list')  # 修改这里以匹配你的URL配置
    else:
        form = ProductForm()
...
def print_order(request, order_id):
    ...
    return render(request, 'order_detail_print.html', context)
